...
def save_result(image_path, conf, classes, threshold):
    ...
    if os.path.exists(output_path):
        ...
        output, _ = non_maximum_suppression(output, threshold)
        v = Visualizer(img[:, :, ::-1],
                       md,
                       scale=1.0,
...
def extract_bbox(image_path, label, threshold,):
    ...
    output = pickle.load(open(output_path, 'rb'))
    output, scores = non_maximum_suppression(output, threshold)    
...
if __name__ == '__main__':
    ...
    for directory in directories:
        files = glob.glob(args.root +'/' + directory + '/pages/*')
        print(args.root, directory+'/pages/*')
        dataset = 'doclaynet'
        interesting_label = ''
